...
\chapter{Programming}
	...
	% http://mywiki.wooledge.org/BashGuide/Patterns	https://en.wikibooks.org/wiki/Bash_Shell_Scripting
		...
		\subsection{Control Structures}
			Like any programming language, bash scripts allow the use of control structures. 
			These structures allow for executing code based on the environment it finds itself in. 
			Examples of basic bash script control structures can be found below:
			\begin{description}
				\item[if]
					The if statement executes its block of code only if the conditional given between its ``\verb+[[]]+'' enclosure resolves to true. 
					These enclosures must be surrounded by spaces.
					The if statement's block should be started with the command ``then'' and ended with the command ``fi''. 
				\item[else]
					When within an if statements block, an else statement will only be executed if the if statement resolves to false. 
			\end{description}
