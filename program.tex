...
\chapter{Network Operations}
	...
	\section{Connecting to Networked Software}
		...
		\subsection{Programming an Interface}
			...
			This will give you a useable interface which will automatically interact with the server. 
			Using this will allow you to automate functions such as handshakes and challenge-response sequences. 

			Using the python code learned in Chapter \ref{ch:Progarmming}, 
...
\chapter{Network Penetration}
	...
	\section{Challenges}
		\subsection{Unsecured FTP to Shell}
			...
			\end{quote}
			Unlike SFTP, the modern protocol, FTP often allows for plain text authentication, meaning that a person intercepting traffic would be able to read the username and password. 
			...
			At this point you should have the username and password. 
			...
			Once you have the username and password, we need to prepare the files that we will be using to get a shell. 
			The first of these is a short python script that will send everything that it is sent to /bin/bash.
			This can be sent as is and can be found in Code example \ref{code:PythonShell}.
			\begin{code}
				\lstinputlisting[language={python}]{./PythonShell.py}
				\caption{Python script to link a socket to a shell}
				\label{code:PythonShell}
			\end{code}
			You will then need to add the following line to their .bashrc:
			\begin{quote}
				/home/msfadmin/PythonShell.py \&
			\end{quote}

			Using FTP with the username and password that you found, run the following commands:
			\begin{verse}
				put PythonShell.py \\
				chmod 777 PythonShell.py \\
				get .bashrc \\
			\end{verse}
...
\chapter{Forensics}
	...
	\section{Wireshark}
		...
		\subsection{The Wireshark UI}
			...
			\begin{figure}[htb]
				\centering
					\includegraphics[scale=0.35]{./WiresharkAnalysis.png}
			...
			\subsection{Kernel Hardening}
				...
				\subsection{Firewall} 
					...
					\subsubsection{Simple Firewall}
						...
						The following commands will stop both of these behaviours. 
						\begin{quote}
