\chapter{Programming}\label{ch:Programming}
	...
	\section{Introduction to the CLI}\index{Linux CLI}
	...
	\section{Introduction to PowerShell}\index{PowerShell}
		%ktiploit.com
	...
	\section{Introduction to Scripting}\index{Bash Scripting}
	% http://mywiki.wooledge.org/BashGuide/Patterns	https://en.wikibooks.org/wiki/Bash_Shell_Scripting
		...
		\subsection{Conditionals}\index{Bash Conditionals}
...
alias ls="ls --color=auto"
			...
			\end{code}
	\section{Introduction to Python}\index{Python}
		...
		\subsection{Objects and Classes}
			...
			\subsubsection{Syntax}
				...
				A common error here is to attempt to use a class before it is defined.
				Unlike some other languages, python requires that code be executed (read by the interpreter) before it can be used.
				When this is happening, python will execute every line of code which is flush with the left margin and anything called by these lines.
				Thus, if you start your main function at the end of the file and create classes flush with the margin, this issue should not be noticed.

				\begin{code}
					\centering
					\pcode{./shellOut/Class.py}
					\caption{Creating a Python Class}\label{code:PythonClass}
				\end{code}

				Classes can be instantiated using another specific syntax which gives a variable which can be used to work with a specific instance of the class.
				This instance is named as any other variable is and retains all information and state given to it.
				However, it does not effect any other instance of the object.
				The syntax for this can be found in code example~\ref{code:ObjectInstantiation}.

				\begin{code}
					\centering
					\pcode{./shellOut/ObjectInstance.py}
					\caption{Creating an Instance of ``myClass''}\label{code:ObjectInstantiation}
				\end{code}

				Data stored within classes has two means of access.
				The first is from within the class itself, in which case, it is called using the special ``self'' keyword.
				When outside the class, an instance or (for static classes) the class name itself is used as the reference.
				This allows for exactly the same syntax to be used within and outside the class.
				Code example~\ref{code:ClassData}

				\begin{code}
					\centering
					\pcode{./shellOut/ClassData.py}
					\caption{Accessing Class Data from Outside a Class}\label{code:ClassData}
				\end{code}

				This same syntax is used for calling a function of a class.
				The only alteration for this is that parenthesis added to the end of the call and variables can be included in these.


	\section{Introducton to JavaScript}\index{Javascript}
...
list = ['one', 'two', 'three'];
	...
	\section{Introduction to C}\index{C}
		...
		\index{Pointers}
			...
			\begin{table}[htb]
				...
				\begin{tabular}{ l  p{8cm} }
					...
					\toprule
					\mintinline{c}|type *ptr| & Create a pointer of name ptr. \\
					\mintinline{c}|*ptr| & The value of whatever ptr is pointing at. \\
					\mintinline{c}|*(ptr + i)| & The value of whatever ptr is pointed at plus i. \\
					\mintinline{c}|&thing| & The address of thing. \\
					\mintinline{c}|type *ptr = &thing| & Create a pointer of type named ptr pointing at the address of thing. \\
					\mintinline{c}|ptr++| & Increment where ptr points. \\
					\bottomrule
				\end{tabular}
				\end{adjustbox}
		...
		\subsection{Pointers to Functions}
			...
			\end{code}
	\section{Debugging with GDB and Peda}\index{GDB}
		...
		however having these makes debugging and reverse engineering easier as you have the working code.\index{PEDA}
		...
		These break points are used to determine the state of variables within the code during execution.
